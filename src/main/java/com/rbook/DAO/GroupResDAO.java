...
public interface GroupResDAO extends Neo4jRepository<GroupResNode, Long> {
	...
	public List<GroupResNodeUserNode> getResListIn(String username, String uuid);

	@Query("MATCH (u:User {username:{0}})<-[:OUT_RES]-(r: GroupRes)<-[:HAS_RES]-(g:Group {uuid:{1}}) WITH r AS res MATCH (res)<-[:IN_RES]-(c: User) RETURN res, c AS user")
	public List<GroupResNodeUserNode> getResListOut(String username, String uuid);
